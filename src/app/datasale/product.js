export const products = [
  {
  id: 1,
  slug: "linkseride-cargo-fat-tyre-electric-bike",

  name: "Linkseride Cargo Fat Tyre Electric Bike",

  image: "/images/Green-Bike-Left-Trans-15-6.png",

  gallery: [
    "/images/Green-Bike-Front-Trans.png",
    "/images/Sale-E-bike-2-Latest-04-06.png",
    "/images/Green-Bike-Right-Trans.png",
  ],

  shortDescription:
    "A practical cargo-style electric bike designed for everyday riding and carrying personal belongings.",

  description:
    "The Linkseride Cargo Fat Tyre Electric Bike features a powerful 750W rear hub motor, a high-capacity 48V 60Ah frame-mounted lithium battery, and an integrated cargo storage system. Built with wide fat tyres, dual-crown suspension, and front and rear disc brakes, it delivers comfort, stability, and utility for daily use.",

  price: 2100,
  oldPrice: 2400,

  tag: "New Arrival",

  specifications: {
    motor: "750W rear hub motor",

    battery: "48V 60Ah frame-mounted lithium battery",

    tyres: '26" × 4.0" fat tyres',

    suspension: "Dual-crown suspension fork",

    brakes: "Front and rear disc brakes",

    storage: "Integrated cargo storage",

    lighting: "LED headlight",

    accessories: "Mudguards included",

    useCase:
      "Suitable for everyday riding and carrying personal belongings",
  },
},
  {
  id: 2,
  slug: "linkseride-otango-retro-fat-tyre-electric-bike",

  name: "Linkseride OTANGO Retro Fat Tyre Electric Bike",

  image: "/images/White-Cycle-Left-Trans.png",

  gallery: [
    "/images/White-Cycle-Front-Trans.png",
    "/images/White-Cycle-Back-Trans.png",
    "/images/Sale-E-bike-3.png",
  ],

  shortDescription:
    "A retro-style electric bike featuring wide fat tyres, a Shimano 7-speed drivetrain, and an integrated lithium battery for comfortable everyday riding.",

  description:
    "The Linkseride OTANGO Retro Fat Tyre Electric Bike is a vintage-inspired electric bicycle designed for comfort and versatility. It features 26 × 4.0-inch fat tyres, a Shimano 7-speed drivetrain, front suspension, front and rear disc brakes, and an integrated lithium battery housed within the retro-style frame.",

  price: 1850,
  oldPrice: 2050,

  tag: "Limited",

  specifications: {
    tyres: '26" × 4.0" fat tyres',

    drivetrain: "Shimano 7-speed drivetrain",

    suspension: "Front suspension",

    brakes: "Front and rear disc brakes",

    battery: "Integrated lithium battery",

    frame: "Retro-style frame",

    included: "One complete electric bicycle",
  },
},
  {
  id: 3,
  slug: "v20-cargo-fat-tyre-electric-bike",

  name: "V20 Cargo Fat Tyre Electric Bike",

  image: "/images/Black-2-Left-04-07.png",

  gallery: [
    "/images/Black-2-Front-04-07.png",
    "/images/Sale-E-bike-4.png",
    "/images/Black-2-Back-04-07.png",
  ],

  shortDescription:
    "A versatile cargo e-bike featuring a 250W rear hub motor, removable battery, hydraulic disc brakes, and long-range pedal assist.",

  description:
    "The V20 Cargo Electric Bike combines utility and comfort with a 250W rear hub motor, Shimano 7-speed gearing, and a removable 48V 15.6Ah lithium-ion battery. Built with puncture-resistant fat tyres, front and rear suspension, and a high payload capacity, it is ideal for commuting, recreational riding, and cargo transport.",

  price: 1450,
  oldPrice: 1650,

  tag: "Hot Deal",

  specifications: {
    motor: "250W rear hub motor",

    battery: "48V 15.6Ah removable lithium-ion battery",

    gears: "Shimano 7-speed gearing",

    tyres: '20" × 4.0" puncture-resistant fat tyres',

    brakes: "Hydraulic disc brakes",

    suspension: "Front and rear suspension",

    range: "Up to 110 km using pedal assist",

    payloadCapacity: "Maximum load capacity of 150 kg",

    useCase:
      "Suitable for commuting, recreational riding and cargo transport",
  },
},

{
  id: 4,
  slug: "hijoe-black-cat-dual-motor-electric-bike",

  name: "HIJOE Black Cat Dual Motor Electric Bike",

  image: "/images/Black-Bike-left-04-07.png",

  gallery: [
    "/images/Black-Bike-back.png",
    "/images/Black-Bike-front-04-07.png",
    "/images/Sale-E-bike-5.png",
  ],

  shortDescription:
    "The HIJOE Black Cat Dual Motor Electric Bike features dual 52V 1000W hub motors, dual 52V 20Ah LG lithium batteries, and a Shimano 9-speed drivetrain.",

  description:
    "Suitable for road riding, long-distance travel, and varied terrain. The HIJOE Black Cat delivers exceptional power, extended range, and all-terrain capability with its dual-motor setup, dual LG battery system, hydraulic suspension, and premium components.",

  price: 3150,
  oldPrice: 3499,

  tag: "New Arrival",

  features: [
    "Dual 1000W hub motors",
    "Dual 52V 20Ah LG lithium batteries",
    "20 × 4.0-inch Kenda fat tyres",
    "Hydraulic disc brakes",
    "Front hydraulic suspension",
    "Rear adjustable suspension",
    "Shimano 9-speed gearing",
    "Up to 150 km pedal-assist range",
  ],

  specifications: {
    motors: "Dual 52V 1000W high-torque hub motors (front and rear)",

    driveModes:
      "Selectable Front-Wheel Drive, Rear-Wheel Drive, or All-Wheel Drive mode",

    controllers: "Dual 52V 30A brushless hall controllers",

    topSpeed: "Unlocked up to 50 km/h (31+ MPH)",

    climbingAngle: "Up to 35° climbing capability",

    batteries:
      "Dual 52V 20Ah LG lithium batteries (40Ah / 2,080Wh total capacity)",

    range:
      "Up to 150 km with pedal assist or 100 km using throttle only",

    charger:
      "59V 5A fast charger (approximately 6–7 hours charging time)",

    brakes:
      "203mm heavy-duty hydraulic disc brakes with automatic motor cut-off",

    gears: "Shimano 9-speed transmission",

    suspension:
      "Hydraulic front fork with rear gas-adjustable suspension",

    tyres: '20" × 4.0" Kenda fat tyres',
  },
},
{
  id: 5,
  slug: "linkseride-moped-style-fat-tyre-electric-bike",

  name: "Linkseride Moped-Style Fat Tyre Electric Bike",

  image: "/images/Black-3-Left-04-07.png",

  gallery: [
    "/images/Black-3-Front-04-07.png",
    "/images/Sale-E-bike-6.png",
    "/images/Black-3-Back-04-07.png",
  ],

  shortDescription:
    "The Linkseride Moped-Style Fat Tyre Electric Bike features a 750W rear hub motor, a 48V 18Ah lithium battery, and a lightweight aluminium alloy frame.",

  description:
    "Suitable for urban riding and recreational cycling. The Linkseride Moped-Style Fat Tyre Electric Bike combines retro-inspired styling with practical performance, offering comfortable seating, powerful assistance, and confident handling across city streets and leisure routes.",

  price: 1600,
  oldPrice: 1899,

  tag: "New Arrival",

  features: [
    '20 × 4.0-inch Kenda fat tyres',
    "750W rear hub motor",
    "48V 18Ah lithium battery",
    "Mechanical disc brakes",
    "Front suspension fork",
    "Long bench seat",
    "Aluminium alloy frame",
  ],

  specifications: {
    motor: "750W rear hub motor",

    battery: "48V 18Ah lithium battery",

    frame: "Lightweight aluminium alloy frame",

    gears: "Single-speed drivetrain",

    brakes: "Mechanical disc brakes",

    suspension: "Front suspension fork",

    tyres: '20" × 4.0" Kenda fat tyres',

    seat: "Long bench seat",

    usage: "Suitable for urban riding and recreational cycling",
  },
}
];